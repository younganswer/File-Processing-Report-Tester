...
timeout_test() {
	...
	if command -v setarch &> /dev/null; then
		setarch $(uname -m) --addr-no-randomize "${PATH_TEST}"/user_exe < "$1" > /dev/null 2> "${PATH_TEST}"/output/${PROJECT}/user_output_test_timeout
	else
...
leak_test() {
	...
	result=$(leaks -atExit -- "${PATH_TEST}"/user_exe < "$2" 2>&1)
